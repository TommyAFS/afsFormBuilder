  ...
  describe('validateDateOfBirth', () => {
    ...
    it('fails when the year is too long ago', () => {
      ...
      expect(result.errors.year).toEqual(
        `Must be under 100 years old.\nEnter a birth year of ${MIN_YEAR_OF_BIRTH} or later.`
      )
    })
    ...
    it('fails when the year is too recent', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors.year).toEqual(
        `Must be at least 14 years old.\nEnter a birth year on or before ${MAX_YEAR_OF_BIRTH_FOR_STUDENT}.`
      )
    })

    it('uses the adult age cutoff in the message for a guarantor / emergency contact', () => {
      const currentYear = new Date().getFullYear()
      const result = validateDateOfBirth(
        { ...validDob, year: currentYear.toString() },
        {
          minYearOfBirth: MIN_YEAR_OF_BIRTH,
          maxYearOfBirth: MAX_YEAR_OF_BIRTH_FOR_AN_ADULT,
          contactType: 'guarantor' as const,
        }
      )
      ...
      expect(result.errors.year).toEqual(
        `Must be at least ${
          currentYear - MAX_YEAR_OF_BIRTH_FOR_AN_ADULT
        } years old.\nEnter a birth year on or before ${MAX_YEAR_OF_BIRTH_FOR_AN_ADULT}.`
      )
    })
    ...
    it('fails with the correct message for a non-numeric day', () => {
      const result = validateDateOfBirth({ ...validDob, day: 'aa' }, options)

      expect(result.isValid).toBe(false)
      expect(result.errors.day).toEqual('Day must be 2 digits')
    })

    it('fails with the correct message for a non-numeric month', () => {
      const result = validateDateOfBirth({ ...validDob, month: 'aa' }, options)

      expect(result.isValid).toBe(false)
      expect(result.errors.month).toEqual('Month must be 2 digits')
    })

    it('fails with the correct message for a non-numeric year', () => {
      const result = validateDateOfBirth({ ...validDob, year: 'abcd' }, options)

      expect(result.isValid).toBe(false)
      expect(result.errors.year).toEqual('Year must be 4 digits')
    })

    it('fails with the correct message for a day out of range', () => {
      const result = validateDateOfBirth({ ...validDob, day: '32' }, options)

      expect(result.isValid).toBe(false)
      expect(result.errors.day).toEqual('Day must be a real date')
    })

    it('fails with the correct message for a month out of range', () => {
      const result = validateDateOfBirth({ ...validDob, month: '13' }, options)

      expect(result.isValid).toBe(false)
      expect(result.errors.month).toEqual('Month must be a real date')
    })
  })

  describe('validateDateOfBirth for a guarantor', () => {
    ...
    it('passes for a valid adult guarantor date of birth', () => {
      const result = validateDateOfBirth(
        { day: '15', month: '06', year: (MIN_YEAR_OF_BIRTH + 10).toString() },
        guarantorOptions
      )
    ...
    it('fails with the correct message when the guarantor is under 18', () => {
      const result = validateDateOfBirth(
        {
          day: '31',
          month: '12',
          year: MAX_YEAR_OF_BIRTH_FOR_AN_ADULT.toString(),
        },
        guarantorOptions
      )
